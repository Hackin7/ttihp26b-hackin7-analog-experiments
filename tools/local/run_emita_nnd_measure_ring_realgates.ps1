...
Write-Host "staged consolidator (gate-subckt + mos-model auto-locate + emit + ngspice) -> ${stage}:"
Get-ChildItem -LiteralPath $stage -Name
Write-Host ""
Write-Host "=== SG13G2 REAL-gate + REAL-mos consolidated ring proof (headless) ==="
